...
TH1D * getEffHist(
	...
	for(int ibin=1; ibin<=nbin; ++ibin) {
		...
		double R = 7.76154e+16/7.41611e+16;  //50 runs segment  basis
	cout << "R:"<<R<<endl;	
		double a = c-(R*d);
		double b = c+(R*d);
		double r = 0;
		double e = 0;
               ...
               // double eNd = h2->GetBinError(ibin);
...
//////
void ssa_seaquest_segments(){
	int limit = 12737+50;
	unsigned int even, odd;
	unsigned int  time_up=0;
	unsigned int  time_down=0;
	unsigned int proton_up=0;
	unsigned int proton_down=0;
	...
	for( Int_t i=0; i<Tree->GetEntries(); i++) {
		...
		time_hour->Fill(time);
	//filing the dimuons in spinup and spindown segments [50 runs wise]
	if (runN <= limit) {
	...
	TF1 *fcos2 = new TF1("fcos2","[0]*([1]*cos(2*x)+1)");
	TF1 *fcos = new TF1("fcos","[0]*([1]*cos(x)+1)");
	TF1 *sin = new TF1("sin","[0]+[1]*sin(x)");
	TF1 *sincos = new TF1("sincos","[0]+[1]*sin(x)+ [2]*cos(2*x)");
	dimu_phi->Fit("fsin");
	...
	cout<< "proton up"<< proton_up << "proton down"<< proton_down<<endl;
